...
.m-lim-sub{font-family:Calibri,Candara,Corbel,sans-serif;font-size:.66em;margin-top:1px;color:var(--mut);}";
        ...
        public static (string kind, string align, string text)? TextDirective(string raw)
        {
            ...
            if (mk == '#')
            {
                ...
                if (Regex.IsMatch(s0, @"^#+\s*(fplot|plot|ezplot|graficas?|grafico|surf|superficie|plot3d|mesh|map|mapa|heatmap|contourf?|beam|viga|esquema|frame|portico)\b", RegexOptions.IgnoreCase)) return null;
            ...
            if (Regex.IsMatch(s, @"^(fplot|plot|ezplot|graficas?|grafico|surf|superficie|plot3d|mesh|map|mapa|heatmap|contourf?|beam|viga|esquema|frame|portico)\b", RegexOptions.IgnoreCase)) return null;
