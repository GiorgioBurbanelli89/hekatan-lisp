...
## 2 · De dónde salen las funciones de forma (se DEDUCEN)
#: Sin carga dentro del tramo la deformada es una CÚBICA (sale de {EI}·v⁗=0, ejemplo 25): {a0 + a1*s + a2*s^2 + a3*s^3}. Tiene 4 coeficientes {a0}…{a3}, y se fijan con los 4 datos de los nudos: descenso y giro en cada extremo. Uso {s} = {x/L}, de 0 a 1.
...
#: Falta el {L}. El giro físico es {theta} = dv/dx, y como {s} = {x/L}, cada derivada en {x} trae un {1/L}: {theta} = {1/L}·dv/ds. Por eso en los datos el giro entra como {L}·{theta}; para que las formas multipliquen al giro {theta} hay que devolver ese {L} en las columnas de giro (2 y 4) con la diagonal diag(1, L, 1, L):
